...
void pwm_init(void) {
    ...
    OCR0B = 0;
    TCCR0A =
        (1 << COM0B1) | (1 << WGM01) | (1 << WGM00);  // non inverting, fast pwm
}
...
void pwm_set(uint8_t val) {
    if (val > 0) {
        OCR0B = val;
        // TCCR0A = (1 << COM0B1) | (1 << WGM01) | (1 << WGM00); // non
        // inverting, fast pwm
        TCCR0B = (1 << CS02);  // clock divider 64: 8 Mhz / 256 / 256 = 122 Hz
                               // PWM frequency

    } else {
        OCR0B = 0;
        while (HW_PORT_IN & (1 << PWM_PIN))
            ;        // auf low warten
        TCCR0B = 0;  // no clock select -> off
