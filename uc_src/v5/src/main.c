...
ISR (PORTA_PORT_vect)
{
    ...
    if (!(MODE_PORT.IN & MODE_PIN_bm)) return; //fallende Flanke
...
void port_init() 
{
    ...
    //PWM Pin low
    PORTA_OUT &= ~PIN5_bm;
    //Port Change interrupt for button
    PORTA.DIR &= ~PIN4_bm; //Pin 4 Input
    PORTA.PIN4CTRL |= PORT_ISC_BOTHEDGES_gc;

    
    //UART Config
    PORTB.DIR = 0x0F;  // Pin 4 und 5 input
...
void main () 
{
    ...
    if (MODE_PORT.IN & MODE_PIN_bm) {
        ...
        setPWM(pwmLevel);
        gotoSleep();
        //hier port Interrupt und gotoSleep
    ...
    while (1) {
        ...
        if (adc_conv_ready) {
            sprintf(buff, "T NTC: %d, T-CPU: %d\n", (uint16_t)NTC_TEMP, getOnChipTemperature() );
            USART0_sendString(buff);
